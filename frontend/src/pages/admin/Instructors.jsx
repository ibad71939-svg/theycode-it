...
export default function Instructors() {
  ...
  const [created, setCreated] = useState(null); // { name, email, note }
  ...
  async function create(e) {
    ...
    try {
      ...
      setCreated({ name: form.name, email: form.email, note: result.note });
      setForm(empty);
      load();
    } catch (err) {
  ...
  {created && (
    <div className="bg-brand-tint border border-brand/30 rounded-admin p-4 mb-6 text-sm">
      <p className="font-semibold text-brand-dark">Profile created for {created.name}</p>
      <p className="mt-1">{created.note || 'No portal login was created for this person.'}</p>
      <button
        onClick={() => setCreated(null)}
        className="block mt-2 text-xs text-muted hover:text-ink"
      >
        Dismiss
      </button>
    </div>
  ...
  <form
    onSubmit={create}
    className="bg-white border-2 border-ink/10 rounded-admin shadow-card p-5 mb-6 grid sm:grid-cols-2 gap-4"
  >
