      ...
      <form onSubmit={create} className="bg-white border border-muted/10 rounded-admin p-5 mb-6 grid sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <Field label="Batch" required>
            <select name="batchId" required className="w-full border border-muted/20 rounded-admin px-3 py-2"
        ...
        <Field label="Due date">
          <input name="dueDate" type="date" className="w-full border border-muted/20 rounded-admin px-3 py-2"
            value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} />
        </Field>
        <Field label="Max marks">
          <input name="maxMarks" type="number" placeholder="Max marks" className="w-full border border-muted/20 rounded-admin px-3 py-2"
